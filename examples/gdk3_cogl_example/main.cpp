...
static void repaint_window(GdkFrameClock *clock, gpointer user_data)
{
    ...
    // Rendering
    cogl_framebuffer_clear4f(COGL_FRAMEBUFFER(ctx->onscreen),
                                                 ...
                                                 void *user_data)
{
    ...
    /* we assume that a GDK event mask is bitwise compatible with X11
       event masks */
    gdk_window_set_events(window, (GdkEventMask) (event_mask |
...
int main(int argc, char** argv)
{
    ...
    win_attrs.event_mask = (GDK_EXPOSURE_MASK |
                            GDK_BUTTON_PRESS_MASK |
                            GDK_BUTTON_RELEASE_MASK |
                            GDK_BUTTON_MOTION_MASK |
                            GDK_KEY_PRESS_MASK |
    ...
    const struct backend_callbacks *cbs = get_backend_callbacks(ctx.window);
    CoglRenderer *renderer = cogl_renderer_new();
    cogl_renderer_set_winsys_id(renderer, cbs->winsys);
    ...
    cbs->set_window(ctx.onscreen, ctx.window);
